...
- Stop the microphone drifting out of saved clips: mic timestamps were anchored once and advanced purely by frame count, so every dropout shifted the remaining timeline earlier by the length of the gap — drift far enough and the mic no longer overlapped the video window and vanished from the clip entirely. Each buffer's host time is now checked against where frame counting says it should land, and a discontinuity past 100ms re-anchors, turning a dropout back into a real gap
- Fix two silent-failure paths around microphone and system audio: starting the mic on a device caught mid-transition (zero sample rate) produced a nil converter and an engine that ran and delivered nothing while reporting success, and a gap in system audio was filled with one silence buffer spanning the whole gap — a 20s stall allocated ~7.7 MB in a single piece, exceeded the audio ring buffer's memory cap by itself, and evicted every real sample ahead of it. Silence is now emitted in bounded chunks and long gaps are left as genuine holes, which the save-time mixer already renders as silence. System audio also gets a stall check, which warns only
- Fix desaturated clips by tagging color end to end: nothing in the pipeline stated a color space, so ScreenCaptureKit's untagged YCbCr stream became an MP4 with no color description and players guessed (usually BT.601) and decoded with the wrong matrix. Capture is pinned to sRGB at every `SCStreamConfiguration` site and carries the tag through live fps/bitrate/resolution updates, the encoded stream is tagged BT.709 primaries/transfer/matrix, and dual-display compositing happens in explicit sRGB. Verified with ffprobe: new clips report bt709 where older ones reported unknown
- Show live audio levels while idle: the meters were only fed from the recording path, so they sat flat whenever nothing was recording. Measurement-only taps now run while the meters are on screen and the pipeline is idle — a separate `AVAudioEngine` tap for the microphone so preview never touches the recording engine's timing state, and an audio-only `SCStream` honouring the same all-apps/per-app filter and exclude-own-audio setting. Volume is applied before normalising so the meters read the same as during recording; preview stands down before the pipeline claims the devices, skips the mic unless access is already granted, and shuts itself off after two seconds with no reader
- Build a universal binary so the app runs on Intel Macs: the release build ran plain `swift build`, which targets only the host architecture, so every shipped binary was arm64-only — and Rosetta translates Intel to ARM, not the reverse, so Intel Macs could not run the app at all. Both build passes and `--show-bin-path` now pass `--arch arm64 --arch x86_64`; `lipo` reports both slices and the signature verifies deep and strict. Recording on Intel is untested, as Quick Sync is a different VideoToolbox path from the Apple Silicon media engine
- Let the clip library hotkey close the library: the shortcut only ever opened the window, so a second press re-focused a library that was already in front. It now closes the window when it is frontmost, and still brings it forward when another app has focus
